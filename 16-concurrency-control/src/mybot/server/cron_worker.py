...
class CronWorker(Worker):
    ...
    async def _dispatch_job(self, job) -> None:
        ...
        # Ensure AgentWorker knows which tools to inject for this cron session
        self._ensure_cron_tools(job, session_id)
    ...
    def _ensure_cron_tools(self, job, session_id: str) -> None:
        """Register per-cron tool overrides (e.g. post_message) in context."""
        if not job.target_session_id:
            return
        # Store the delivery session so AgentWorker can inject post_message
        if not hasattr(self.context, "cron_delivery_map"):
            self.context.cron_delivery_map = {}
        self.context.cron_delivery_map[session_id] = job.target_session_id
