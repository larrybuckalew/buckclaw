"""cli/main.py -- Entry point for Step 17: Memory."""
...
from mybot.tools.memory_tools import ListMemoriesTool, ReadMemoryTool, WriteMemoryTool
...
def server(
    config: Optional[Path] = typer.Option(
        None, "--config", "-c", file_okay=True, dir_okay=False
    ),
) -> None:
...
def chat(
    ...
    if config:
        workspace_dir = config.parent
    asyncio.run(
        _run_server(workspace_dir, project_root, with_websocket=False, cli_only=True)
    )
...
async def _run_server(
    ...
    # Memories live inside the workspace so the memory agent can reach them
    # via the sandboxed read/write tools (which are rooted at project_root).
    memory_root  = project_root / "default_workspace" / "memories"
    ...
    tools_list = [
        ...
        skill_tool,
        # Memory tools -- scoped to memories/ directory
        ReadMemoryTool(memory_root=memory_root),
        WriteMemoryTool(memory_root=memory_root),
        ListMemoriesTool(memory_root=memory_root),
    ]
    ...
    context = AppContext(
        ...
        history_store=history_store, tool_registry=tool_registry,
        skill_loader=skill_loader, agent_loader=agent_loader,
    ...
    delivery_worker = DeliveryWorker(
        context=context, channels=channels, source_map=context.session_source_map
    )
    ...
    if cfg.heartbeat.enabled:
        hb_meta = history_store.create_session(
            agent_id="heartbeat", agent_name="Heartbeat"
        )
    ...
    console.print(f"[dim]Memory store: {memory_root}[/dim]")
    console.print()
