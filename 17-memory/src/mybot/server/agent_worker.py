...
class AgentWorker(Worker):
    ...
    Concurrency control: each AgentDef may declare max_concurrency > 0 to
    cap how many sessions for that agent type may run simultaneously.  When
    the cap is reached, subsequent requests block (rather than being dropped)
    ...
    def __init__(self, context: AppContext) -> None:
        ...
        # Per-agent semaphores.  Keys are agent ids; value is the Semaphore.
    ...
    def _get_or_create_semaphore(self, agent_id: str, max_concurrency: int) -> asyncio.Semaphore | None:
        """Return a semaphore for *agent_id*, or None when unlimited."""
        if max_concurrency == _UNLIMITED:
            return None
        if agent_id not in self._semaphores:
            self._semaphores[agent_id] = asyncio.Semaphore(max_concurrency)
            logger.debug(
                "Created semaphore for agent '%s' with limit %d",
                agent_id,
                max_concurrency,
            )
        return self._semaphores[agent_id]

    def _maybe_cleanup_semaphore(self, agent_id: str) -> None:
        """Remove semaphore once no waiters remain (keeps dict tidy on hot-reload)."""
        sem = self._semaphores.get(agent_id)
        if sem is not None and sem._value == (sem._value + len(sem._waiters) - len(sem._waiters)):
            # Only remove if nobody is currently waiting
            if not getattr(sem, "_waiters", None):
                self._semaphores.pop(agent_id, None)

    # ------------------------------------------------------------------
    # Core session execution (semaphore-aware)
    # ------------------------------------------------------------------

    async def _exec_session(self, session_id: str, content: str, agent_id: str, max_concurrency: int) -> str:
        """Run a single chat turn, honouring concurrency limits for *agent_id*."""
        sem = self._get_or_create_semaphore(agent_id, max_concurrency)

        if sem is not None:
            waiters_before = len(getattr(sem, "_waiters", []))
            if waiters_before > 0 or sem.locked():
                logger.warning(
                    "Agent '%s' at concurrency limit (%d). "
                    "Session '%s' is waiting for a free slot.",
                    agent_id,
                    max_concurrency,
                    session_id,
                )
            async with sem:
                result = await self._run_chat(session_id, content)
    ...
    async def _run_chat(self, session_id: str, content: str) -> str:
        """Low-level: call agent.chat() for this session."""
    ...
    async def handle_inbound(self, event: InboundEvent) -> None:
        ...
        try:
            ...
            agent_id, max_concurrency = self._resolve_agent_concurrency(event.session_id)
            response = await self._exec_session(
                event.session_id, event.content, agent_id, max_concurrency
            )
    ...
    async def handle_dispatch(self, event: DispatchEvent) -> None:
        """Process a cron/subagent dispatch -- publishes DispatchResultEvent."""
        try:
            if event.source:
                self.context.session_source_map[event.session_id] = event.source
    ...
    def _resolve_agent_concurrency(self, session_id: str) -> tuple[str, int]:
        """Return (agent_id, max_concurrency) for a given session."""
        agent_id = "my-bot"
        max_concurrency = _UNLIMITED
        ...
        return agent_id, max_concurrency
    ...
    def _get_or_create_session(self, session_id: str):
        ...
        if session_id not in self._sessions:
            ...
            if delivery_session_id:
                ...
                logger.info(
                    "Injected post_message for cron session %s -> %s",
                    session_id,
                    delivery_session_id,
                )
            ...
            if self.context.agent_loader:
                ...
                if dispatch_tool:
                    ...
                    tools = ToolRegistry(all_tools)
                    logger.debug(
                        "Injected subagent_dispatch for agent %s", current_agent_id
                    )
