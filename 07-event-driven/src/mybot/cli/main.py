"""cli/main.py -- Event-driven entry point for Step 07."""
...
app = typer.Typer(name="my-bot", help="BuckClaw tutorial bot -- Step 07: Event-Driven", add_completion=False)
console = Console()


@app.command()
def chat(
    config: Optional[Path] = typer.Option(None, "--config", "-c", exists=True, file_okay=True, dir_okay=False),
) -> None:
    """Start an interactive chat session (event-driven)."""
    try:
    ...
    asyncio.run(_run(cfg, config))


async def _run(cfg, config_path) -> None:
    ...
    cli_adapter  = CLIAdapter(context=context, session_id=session_id, console=console)

    # Start background workers
    await eventbus.start()
    await agent_worker.start()

    # Run CLI in foreground (it exits when user types quit)
    try:
